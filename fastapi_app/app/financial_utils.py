...
def safe_float(val):
    try:
        if (
            val is None
            or val == ""
            or (isinstance(val, str) and val.strip().lower() in ["nan", "null", "none"])
        ):
            return None
        return float(val)
    except Exception:
...
class FinancialDataManager:
    ...
    def __init__(self):
        ...
        if not all([self.project_id, self.dataset, self.table]):
            ...
            raise ValueError(
                f"Missing required BigQuery environment variables: {', '.join(missing_vars)}"
            )
    ...
    def _initialize_bigquery_client(self):
        ...
        if credentials_path and os.path.exists(credentials_path):
            self.bq_client = bigquery.Client.from_service_account_json(
                credentials_path, project=self.project_id, location=self.location
            )
            logger.info(
                f"BigQuery client initialized with service account file: {credentials_path}"
            )
    ...
    def load_metadata_from_bigquery(self):
        ...
        try:
            ...
            companies = [
                ...
                for row in self.bq_client.query(companies_query).result()
            ]
            ...
            """
            symbols = [
                get_row_attr(row, "Symbol") for row in self.bq_client.query(symbols_query).result()
            ]
            ...
            """
            years = [
                get_row_attr(row, "Year") for row in self.bq_client.query(years_query).result()
            ]
            ...
            """
            standard_items = [
                get_row_attr(row, "standard_item")
                for row in self.bq_client.query(items_query).result()
            ]
            ...
            """
            company_to_symbol = {
                get_row_attr(row, "Company"): get_row_attr(row, "symbols")
                for row in self.bq_client.query(c2s_query).result()
            }
            # symbol_to_company
            s2c_query = f"""
            ...
            """
            symbol_to_company = {
                get_row_attr(row, "Symbol"): get_row_attr(row, "companies")
                for row in self.bq_client.query(s2c_query).result()
            }
            # company_to_years
            c2y_query = f"""
            ...
            """
            company_to_years = {
                get_row_attr(row, "Company"): get_row_attr(row, "years")
                for row in self.bq_client.query(c2y_query).result()
            }
            # company_to_items
            c2i_query = f"""
            ...
            """
            company_to_items = {
                get_row_attr(row, "Company"): get_row_attr(row, "items")
                for row in self.bq_client.query(c2i_query).result()
            }
            # year_to_companies
            y2c_query = f"""
            ...
            """
            year_to_companies = {
                get_row_attr(row, "Year"): get_row_attr(row, "companies")
                for row in self.bq_client.query(y2c_query).result()
            }
            # item_to_companies
            i2c_query = f"""
            ...
            """
            item_to_companies = {
                get_row_attr(row, "standard_item"): get_row_attr(row, "companies")
                for row in self.bq_client.query(i2c_query).result()
            }
            ...
            """
            year_to_items = {
                get_row_attr(row, "Year"): get_row_attr(row, "items")
                for row in self.bq_client.query(y2i_query).result()
            }
            ...
            logger.info(
                f"Filtered data - companies: {len(companies_filtered)}/{len(companies)}, symbols: {len(symbols_filtered)}/{len(symbols)}, years: {len(years_filtered)}/{len(years)}, items: {len(standard_items_filtered)}/{len(standard_items)}"
            )
    ...
    def parse_user_query(
        ...
        try:
            ...
            if result.get("is_follow_up") and last_query_data:
                ...
                if (
                    result.get("standard_items")
                    and not result.get("companies")
                    and not result.get("symbols")
                ):
                    result["companies"] = last_filters.get("companies", [])
                    result["symbols"] = last_filters.get("symbols", [])
    ...
    def _post_process_filters(self, result: Dict) -> Dict:
        ...
        if result.get("symbols"):
            ...
            if companies:
                ...
                logger.info(
                    f"Found companies for symbols {result['symbols']}: {', '.join(result['companies'])}"
                )
            elif result["symbols"]:
                result["companies"] = []
    ...
    def _fallback_parse_query(
        self, query: str, last_query_data: Optional[Dict] = None
    ) -> FinancialDataFilters:
    ...
    def query_data(self, filters: FinancialDataFilters) -> List[FinancialDataRecord]:
        ...
        try:
            ...
            for row in results:
                ...
                calculated_value = (
                    safe_float(get_row_attr(row, "calculated_value"))
                    if (hasattr(row, "calculated_value") or "calculated_value" in dir(row))
                    and get_row_attr(row, "calculated_value") is not None
                    else None
                )
                # Format value
                ...
                record = FinancialDataRecord(
                    company=str(
                        get_row_attr(row, "company")
                        if hasattr(row, "company") or "company" in dir(row)
                        else get_row_attr(row, "Company")
                    ),
                    symbol=str(
                        get_row_attr(row, "symbol")
                        if hasattr(row, "symbol") or "symbol" in dir(row)
                        else get_row_attr(row, "Symbol")
                    ),
                    year=str(
                        get_row_attr(row, "year")
                        if hasattr(row, "year") or "year" in dir(row)
                        else get_row_attr(row, "Year")
                    ),
    ...
    def validate_data_availability(self, filters: FinancialDataFilters) -> Dict[str, Any]:
        ...
        if filters.companies and filters.years and filters.standard_items:
            ...
            if "company_year_to_items" in self.metadata["associations"]:
                for company in filters.companies:
                    ...
                    company_year_items = (
                        self.metadata["associations"]
                        .get("company_year_to_items", {})
                        .get(company, {})
                    )
                    ...
                    for year in filters.years:
                        ...
                        if year not in company_year_items:
                            available_years = (
                                self.metadata["associations"]
                                .get("company_to_years", {})
                                .get(company, [])
                            )
                            if available_years:
                                available_alternatives[company] = available_years[
                                    -3:
                                ]  # Last 3 years
                            ...
                            elif hasattr(available_items, "items") and not callable(
                                available_items.items
                            ):
                                available_items = available_items.items
                                logger.info(
                                    "Extracted available_items using attribute access (not callable)"
                                )
                            elif hasattr(available_items, "items") and callable(
                                available_items.items
                            ):
                                logger.warning(
                                    "available_items is still a method after attempted extraction; falling back to empty list."
                                )
                                available_items = []
                            else:
                                logger.warning(
                                    "Could not extract 'items' from available_items; falling back to empty list."
                                )
                                available_items = []
                            logger.info(
                                f"available_items type: {type(available_items)}, available_items: {available_items}"
                            )
        ...
        # Check if items are available for ANY of the requested companies
            ...
            for item in filters.standard_items:
                logger.info(f"Checking item: {item}")
                item_companies = set(
                    self.metadata["associations"].get("item_to_companies", {}).get(item, [])
                )
    ...
    def _should_recommend_deep_research(
        self, records: List[FinancialDataRecord], query: str, interpretation: str
    ) -> bool:
        ...
        if any(
            keyword in query_lower or keyword in interpretation_lower
            for keyword in detailed_query_keywords
        ):
            return True
