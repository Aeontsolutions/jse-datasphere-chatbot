...
from app.models import StreamingChatRequest, FinancialDataResponse, FinancialDataFilters
...
async def process_streaming_financial_chat(
    ...
    asyncio.create_task(_process_financial_chat_async(
        request, financial_manager, tracker
    ))
...
async def _process_financial_chat_async(
    ...
    try:
        ...
        if not financial_manager:
            await tracker.emit_error("Financial data service is not available. Please ensure BigQuery is configured.")
            return
        ...
        await tracker.emit_progress(
            ...
            25.0,
            {"companies": filters.companies, "years": filters.years, "metrics": filters.standard_items}
        )
