    ...
    def query(self, query, job_config=None):
        ...
        # Otherwise, return filtered rows for data queries
            ...
            if (
                ...
                def row_matches(row):
                    if (
                        "companies" in filters
                        and getattr(row, "company", None) not in filters["companies"]
                    ):
                        return False
                    if (
                        "symbols" in filters
                        and getattr(row, "symbol", None) not in filters["symbols"]
                    ):
                        return False
                    if (
                        "years" in filters
                        and str(getattr(row, "year", None)) not in filters["years"]
                    ):
                        return False
                    if (
                        "items" in filters
                        and getattr(row, "standard_item", None) not in filters["items"]
                    ):
                        return False
                    return True
