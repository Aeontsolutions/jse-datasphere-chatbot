...
from app.utils import (
    ...
    _download_single_document_async,
...
class TestAsyncDocumentDownload:
    ...
    async def test_download_and_extract_success(self, mock_extract_text, mock_init_client, 
                                              download_config, mock_progress_callback):
        ...
        mock_progress_callback.assert_any_call("download_start", "Starting download of test-file.pdf")
        mock_progress_callback.assert_any_call("extraction_complete", f"Extracted {len(MOCK_EXTRACTED_TEXT)} characters")
    ...
    @pytest.mark.asyncio
    @patch('app.utils.init_async_s3_client')
    async def test_download_with_retry(self, mock_init_client, download_config, mock_progress_callback):
    ...
    @pytest.mark.asyncio
    @patch('app.utils.init_async_s3_client')
    async def test_download_max_retries_exceeded(self, mock_init_client, download_config, mock_progress_callback):
...
class TestAsyncMetadataDownload:
    ...
    @pytest.mark.asyncio
    @patch('app.utils.init_async_s3_client')
    async def test_download_metadata_success(self, mock_init_client, download_config, mock_progress_callback):
        ...
        mock_progress_callback.assert_any_call("metadata_download_start", "Downloading metadata: metadata.json")
        mock_progress_callback.assert_any_call("metadata_download_complete", f"Downloaded metadata ({len(MOCK_METADATA_JSON)} bytes)")
    
    @pytest.mark.asyncio
    @patch('app.utils.init_async_s3_client')
    async def test_download_metadata_invalid_json(self, mock_init_client, download_config, mock_progress_callback):
...
class TestConcurrentDocumentLoading:
    ...
    async def test_concurrent_document_loading_success(self, mock_download, mock_selection,
                                                     mock_metadata, download_config, mock_progress_callback):
    ...
    @patch('app.utils.semantic_document_selection')
    @patch('app.utils.download_and_extract_from_s3_async')
    async def test_concurrent_loading_partial_failure(self, mock_download, mock_selection,
                                                    mock_metadata, download_config, mock_progress_callback):
    ...
    async def test_semaphore_limits_concurrency(self):
        ...
        tasks = [
            _download_with_semaphore(semaphore, mock_download_task())
            for _ in range(5)
        ]
...
class TestLoadMetadataFromS3Async:
    ...
    async def test_load_metadata_success(self, mock_parse, mock_download):
        ...
        mock_download.assert_called_once_with("test-bucket", "metadata.json", None, mock_progress_callback)
    ...
    async def test_load_metadata_missing_bucket_env(self):
        ...
        assert result is None
        mock_progress_callback.assert_called_with("metadata_error", "DOCUMENT_METADATA_S3_BUCKET not found in environment variables")