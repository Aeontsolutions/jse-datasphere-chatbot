...
class TestQueryCollectionFilenameFiltering:
    ...
    def setup_method(self):
        ...
        self.mock_collection.query.return_value = {
            ...
            "metadatas": [
                [
                    {"year": "2023", "filename": "test1.pdf"},
                    {"year": "2022", "filename": "test2.pdf"},
                ]
            ],
        }
    ...
    def test_fallback_behavior_with_filter(self):
        ...
        self.mock_collection.query.side_effect = [
            {"ids": [[]], "documents": [[]], "metadatas": [[]]},  # No results with filter
            {
                "ids": [["doc1"]],
                "documents": [["Content"]],
                "metadatas": [[{"year": "2023", "filename": "test.pdf"}]],
...
class TestQueryCollectionRobustness:
    ...
    def test_query_with_changing_filters(self):
        ...
        result1, context1 = query_collection(
            self.mock_collection, "query", n_results=5, where=filter1
        )
        result2, context2 = query_collection(
            self.mock_collection, "query", n_results=5, where=filter2
        )
