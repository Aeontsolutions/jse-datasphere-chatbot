...
class MockPlatformClient:
    ...
    def send_query(
        ...
        try:
            with self.session.post(
                self.base_url, json=payload, headers=headers, stream=True, timeout=120
            ) as resp:
            ...
            for chunk in resp.iter_content(chunk_size=1024):
                ...
                for line in lines:
                    ...
                    if line.startswith("event: "):
                        ...
                        normalized_event = (
                            event.strip() if event and event.strip() != "" else "message"
                        )
    ...
    def _poll_job_status(
        self, job_payload: Dict[str, Any], base_url: str
    ) -> Generator[Tuple[str, str], None, None]:
        """
    ...
    def _transform_job_status_to_events(
        ...
        if latest_progress:
            if isinstance(latest_progress, dict):
                ...
                logger.info(
                    f"📈 Progress: {progress_data['message']} ({progress_data['progress']}%)"
                )
...
Examples:
  ...
  python mock_client.py --url https://chatbot.com/api/v1/stream --query "Test" --verbose
    ...
    parser.add_argument(
        "--url", required=True, help="Chatbot service URL (e.g., https://chatbot.com/api/v1/stream)"
    )
    parser.add_argument("--query", required=True, help="Query to send to chatbot")
    parser.add_argument("--history", help="Path to JSON file with conversation history")
    parser.add_argument(
        "--poll-interval",
        type=float,
    ...
    parser.add_argument(
        "--no-auto-load", action="store_true", help="Disable automatic document loading"
    )
    ...
    parser.add_argument("--save-output", help="Save events to JSON file")
