    ...
    steps = [
        ...
        },
        {
            ...
            "data": {
                ...
                "progress": 50,
                "details": {"metrics_analyzed": 15, "total_metrics": 30},
            },
        },
        {
            "event": "progress",
...
def stream_endpoint():
    ...
    try:
        ...
        if mode == "job":
            ...
            response = Response(
                generate_sse_stream(query, conversation_history), mimetype="text/event-stream"
            )
...
def create_job_response(query: str, conversation_history: list) -> tuple:
    ...
    response_data = {"job_id": job_id, "status": "pending", "polling_url": f"/jobs/{job_id}"}

    # Set session cookie
    response = make_response(jsonify(response_data))
    response.set_cookie("session_id", session.get("session_id"), httponly=True, samesite="Lax")
...
def get_job_status(job_id: str):
    ...
    if job["session_id"] != client_session:
        logger.warning(
            f"⚠️ Session mismatch! Job session: {job['session_id']}, Client: {client_session}"
        )
    ...
    if current_status == "pending":
        ...
        if current_progress < 50:
            ...
            job["result"] = {
                ...
                "metadata": {"processing_time": 6.2, "documents_analyzed": 8, "job_id": job_id},
            }
...
def health():
    ...
    return jsonify(
        {"status": "healthy", "timestamp": datetime.utcnow().isoformat(), "active_jobs": len(jobs)}
    )
